...
async function main() {
  ...
  const firstRegistrationStart = randomDate(new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000), new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000));
  const firstRegistrationEnd = randomDate(firstRegistrationStart, new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000));
  const firstStartDate = randomDate(firstRegistrationEnd, new Date(now.getTime() + 60 * 24 * 60 * 60 * 1000));
  ...
  if (firstEndDate < now) {
    firstStatus = randomChoice(['ARCHIVED', 'CANCELLED']);
  } else if (firstStartDate > now) {
    firstStatus = 'DRAFT';
  }
  ...
  for (let i = 1; i < 50; i++) {
    ...
    const registrationStart = randomDate(new Date(now.getTime() - 60 * 24 * 60 * 60 * 1000), new Date(now.getTime() - 1 * 24 * 60 * 60 * 1000));
    const registrationEnd = randomDate(registrationStart, new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000));
    const startDate = randomDate(registrationEnd, new Date(now.getTime() + 60 * 24 * 60 * 60 * 1000));
    const endDate = new Date(startDate.getTime() + randomInt(1, 7) * 24 * 60 * 60 * 1000);
    const judgingStart = endDate;
    const judgingEnd = new Date(judgingStart.getTime() + randomInt(3, 7) * 24 * 60 * 60 * 1000);
    
    // Determine status based on dates
    let status: 'DRAFT' | 'ACTIVE' | 'ARCHIVED' | 'CANCELLED' = 'ACTIVE';
    if (endDate < now) {
      status = randomChoice(['ARCHIVED', 'CANCELLED']);
  ...
  for (const hackathon of hackathons) {
    const sponsorCountForHackathon = randomInt(2, 5);
    const sponsorNames = ['Chainlink', 'The Graph', 'Aave', 'Uniswap', 'Polygon', 'Arbitrum'];
    
    for (let i = 0; i < sponsorCountForHackathon; i++) {
      ...
      // Create 1-2 bounties per sponsor
